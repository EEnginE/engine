...
/*!
 * \brief Constructor from SHA_2
 * 
 * \note If _type is not a SHA2 function, SHA-256 wil be used
 */
SHA_2::SHA_2( HASH_FUNCTION _type ) {
...
std::vector< unsigned char > SHA_2::quickHash( HASH_FUNCTION _type, std::string _message ) {
   reset( _type );
   add( _message );
   return end();
}
...
std::vector< unsigned char > SHA_2::quickHash( HASH_FUNCTION _type, std::vector< unsigned char > _binary ) {
   reset( _type );
   add( _binary );
   return end();
}
 ...
 */
void SHA_2::reset( HASH_FUNCTION _type ) {
   if ( _type == SHA2_224 || _type == SHA2_256 || _type == SHA2_384 || _type == SHA2_512 ) {
      vType             = _type;
   }
   vBlockCounter_ulI = 0;
 ...
 */
std::vector<unsigned char> SHA_2::end() {
   if ( vEnded_B )
...
unsigned int SHA_2::getHashLength() {
   switch( vType ) {
