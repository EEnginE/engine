 ...
 */
void SHA_2::block( std::array<unsigned char, 64> const &_data ) {
   ...
   for ( t = 0; t < 16; ++t ) {
      word[t] = ( ( uint32_t )_data[t * 4 + 0] << 24 ) +
                ( ( uint32_t )_data[t * 4 + 1] << 16 ) +
                ( ( uint32_t )_data[t * 4 + 2] << 8 )  +
                ( ( uint32_t )_data[t * 4 + 3] );
   }
