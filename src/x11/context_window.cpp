...
// Set new Error Handler
   const GLushort VERSION_COUNT = 14;
   GLushort version_list[VERSION_COUNT][2] = {
      {4, 3},
      {4, 2},
      {4, 1},
      {4, 0},
   ...
   if ( !isExtensionSupported( "GLX_ARB_create_context" ) || !glXCreateContextAttribsARB ) {
      ...
      if ( WinData.versions.glMinorVersion < 0 || WinData.versions.glMajorVersion < 0 ) {
         lAttributes_A_I[0] = 0;
      } else {
      ...
      for ( unsigned short int i = 0; i < VERSION_COUNT; i++ ) {
         ...
         if ( gContextErrorOccoured_B == true || !vOpenGLContext_GLX ) {
            while ( version_list[i][0] >= lAttributes_A_I[1] && version_list[i][1] >= lAttributes_A_I[3] ) {
               i++;
            }
