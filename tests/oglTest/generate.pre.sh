...
void ${CLASS_NAME}::enable( string _test ) {
   for( unsigned int i = 0; i < $NUM_OF_TESTS; ++i ) {
      if( tests[i] == _test ) {
         doThisTest[i] = true;
         iLOG "Test '" ADD tests[i] ADD "' enabled" END
         return;
      }
   }
   wLOG "Unable to find test '" ADD _test ADD "' to enable it" END
}
...
void ${CLASS_NAME}::disable( string _test ) {
   for( unsigned int i = 0; i < $NUM_OF_TESTS; ++i ) {
      if( tests[i] == _test ) {
         doThisTest[i] = false;
         iLOG "Test '" ADD tests[i] ADD "' disabled" END
         return;
      }
   }
   wLOG "Unable to find test '" ADD _test ADD "' to disable it" END
}
...
cat >> $CPP_FILE << EOF
   ...
   if( doThisTest[$i] ) {
      dLOG "" END
      dLOG "      STARTING TEST '${TEST}'" END
      dLOG "      ===============${EQUAL_SIGNS}=" END
      dLOG "" END
      dLOG "  -- " ADD ${TEST}::desc END
      dLOG "" END
...
for (( i=0; i < NUM_OF_TESTS; ++i )); do
   ...
   echo "   dLOG \" - ${TEST}${SPACE}-- \" ADD ${TEST}::desc END" >> $CPP_FILE
done
