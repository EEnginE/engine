...
struct uJSON_data {
  ...
  inline void _(
      uJSON_data *_first, JSON_DATA_TYPE _type, T *_pointer, D const &_setData, END_MARKER_TYPE _what, ARGS... _args);
...
void uJSON_data::_(
    uJSON_data *_first, JSON_DATA_TYPE _type, T *_pointer, D const &_setData, END_MARKER_TYPE _what, ARGS... _args) {
  if (type == __JSON_NOT_SET__ || !_pointer || (type != _type && !(type == JSON_INT && _type == JSON_NUMBER)))
    _what = SET;
