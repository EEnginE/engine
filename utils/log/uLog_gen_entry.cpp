...
void testLogSize(uLogEntryRaw *data, unsigned int _maxTypeStringLength) {
  ...
  unsigned int lThisSize = lTimeSize_uI + lFileSize_uI + lThreadSize_uI + _maxTypeStringLength + 2 + // '[' and ']'
                           75;                                                                       // The main Message
...
void uLogEntryRaw::defaultEntryGenerator() {
  ...
  if (data.config.vTime_LPT != OFF) {
    ...
    if (data.config.vTime_LPT == LEFT_FULL || data.config.vTime_LPT == RIGHT_FULL) {
      lTime_str += numToSizeString(ltemp_TM->tm_year + 1900, 4, L'0') + L'-' +
                   numToSizeString(ltemp_TM->tm_mon + 1, 2, L'0') + L'-' + numToSizeString(ltemp_TM->tm_mday, 2, L'0') +
                   L' ';
    }

    lTime_str += numToSizeString(ltemp_TM->tm_hour, 2, L'0') + L':' + numToSizeString(ltemp_TM->tm_min, 2, L'0') +
                 L':' + numToSizeString(ltemp_TM->tm_sec, 2, L'0') + lResetColl_STR;
  }
    ...
    if (data.config.vFile_LPT == LEFT_FULL || data.config.vFile_LPT == RIGHT_FULL) {
      ...
      if (data.config.vColor_LCT == FULL || (data.raw.vBold_B == true && data.config.vColor_LCT != DISABLED)) {
        lFile_str += eCMDColor::color(eCMDColor::BOLD, eCMDColor::YELLOW) + L':' +
                     ...
                     data.raw.vFunctionName_STR + eCMDColor::color(eCMDColor::BOLD, eCMDColor::YELLOW) + L':' +
  ...
  if (data.config.vThread_LPT != OFF) {
    ...
    if (data.config.vColor_LCT == FULL || (data.raw.vBold_B == true && data.config.vColor_LCT != DISABLED)) {
      lThread_str = eCMDColor::color(eCMDColor::BOLD, eCMDColor::BLUE) + L" [" +
                    ...
                    lTempThread_str + eCMDColor::color(eCMDColor::BOLD, eCMDColor::BLUE) + L"] ";
    } else {
  ...
  unsigned int lMaxFileSize =
      GlobConf.log.maxFilenameSize + ((data.config.vFile_LPT == LEFT_FULL || data.config.vFile_LPT == RIGHT_FULL)
  ...
  if (data.config.vColumns_I < 0) {
    ...
    lErrTypeD_I =
        data.config.vErrorType_LPT == RIGHT_FULL || data.config.vErrorType_LPT == RIGHT_REDUCED ? 1 : lErrTypeD_I;
  ...
  if ((static_cast<int>(data.config.vColumns_I) - static_cast<int>(lLeftL_uI) - static_cast<int>(lRightL_uI)) < 0 &&
  ...
  if (data.config.vColumns_I > 0)
    lMaxMessageSize_uI = (static_cast<size_t>(data.config.vColumns_I) - lLeftL_uI - lRightL_uI);
