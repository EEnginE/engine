...
void testLogSize( uLogEntry &_rawEntry, unsigned int _maxTypeStringLength ) {
...
void uLog::generateEntry( uLogEntry &_rawEntry ) {
...
#if UNIX
      if ( _rawEntry.config.vColor_LCT == FULL && _rawEntry.data.vBold_B == false )
         _rawEntry.temp.vTime_STR += eCMDColor::color( eCMDColor::OFF,  GlobConf.log.standardTimeColor );

      else if ( _rawEntry.config.vColor_LCT == FULL && _rawEntry.data.vBold_B == true )
         _rawEntry.temp.vTime_STR += eCMDColor::color( eCMDColor::BOLD, GlobConf.log.standardTimeColor );
...
// ========= Generate The File Entry ==============================================================================================================
   ...
   if ( _rawEntry.config.vFile_LPT != OFF ) {
      ...
      if ( _rawEntry.config.vFile_LPT == LEFT_FULL || _rawEntry.config.vFile_LPT == RIGHT_FULL ) {
         if ( _rawEntry.data.vFunctionName_STR.size() > GlobConf.log.maxFunctionNameSize )
            _rawEntry.data.vFunctionName_STR.resize( GlobConf.log.maxFunctionNameSize );
