...
void uSHA_2::block(std::array<unsigned char, 64> const &_data) {
  ...
  for (t = 0; t < 16; ++t) {
    word[t] = (static_cast<uint32_t>(_data[t * 4 + 0]) << 24) + (static_cast<uint32_t>(_data[t * 4 + 1]) << 16) +
              (static_cast<uint32_t>(_data[t * 4 + 2]) << 8) + (static_cast<uint32_t>(_data[t * 4 + 3]));
  }


  for (; t < 64; ++t) { word[t] = S1(word[t - 2]) + word[t - 7] + S0(word[t - 15]) + word[t - 16]; }
...
void uSHA_2::padd512() {
  uint32_t lElementsInBuffer_uI = static_cast<uint32_t>(vCurrentPos512_A_IT - vBuffer512_A_uC.begin());
