...
void uSHA_2::block(std::array<unsigned char, 128> const &_data) {
  ...
  for (t = 0; t < 16; ++t) {
    word[t] = (static_cast<uint64_t>(_data[t * 8 + 0]) << 56) + (static_cast<uint64_t>(_data[t * 8 + 1]) << 48) +
              (static_cast<uint64_t>(_data[t * 8 + 2]) << 40) + (static_cast<uint64_t>(_data[t * 8 + 3]) << 32) +
              (static_cast<uint64_t>(_data[t * 8 + 4]) << 24) + (static_cast<uint64_t>(_data[t * 8 + 5]) << 16) +
...
void uSHA_2::padd1024() {
  uint64_t lElementsInBuffer_uI = static_cast<uint64_t>(vCurrentPos1024_A_IT - vBuffer1024_A_uC.begin());
