...
int iContext::enableVSync() {
  ...
  if (wglewIsSupported("WGL_EXT_swap_control")) {
    if (wglSwapIntervalEXT(1) != TRUE) {
      switch (GetLastError()) {
        case ERROR_INVALID_DATA:
          wLOG(
              "VSync Error [WGL] ERROR_INVALID_DATA; 1 seams to be not a good value on this "
              "System\n==> VSync NOT enabled");
          return 3;
        case ERROR_DC_NOT_FOUND:
          wLOG(
              "VSync Error [WGL] ERROR_DC_NOT_FOUND; There is no *current* OpenGL context "
              "in this thread. Use makeContextCurrent() to fix this\n==> VSync NOT "
              "enabled");
          return 4;
        default:
          wLOG(
              "VSync Error [WGL] <UNKNOWN>; Unknown return value of glXSwapIntervalSGI\n==> "
              "VSync NOT enabled");
          return 5;
      }
    }
    ...
    wLOG(
        "VSync Error [WGL]; Extention WGL_EXT_swap_control not supported\n==> VSync NOT "
        "enabled");
    return 2;
  }
}
  ...
  if (wglewIsSupported("WGL_EXT_swap_control")) {
    if (wglSwapIntervalEXT(0) != TRUE) {
      switch (GetLastError()) {
        case ERROR_INVALID_DATA:
          wLOG(
              "VSync Error [WGL] ERROR_INVALID_DATA; 0 seams to be not a good value on this "
              "System\n==> VSync NOT disabled");
          return 3;
        case ERROR_DC_NOT_FOUND:
          wLOG(
...
bool iContext::makeContextCurrent() {
  if (!vHasContext_B) {
    eLOG(
        "OpenGL context Error [WGL]; We do not have any context. Please create it with "
        "iInit::init() before you run this!");
    return false;
  }
  if (vAThreadOwnsTheOpenGLContext_B) {
    eLOG(
        "The OpenGL Context is already in use in another or this thread! Cannot make it "
        "current now!");
    return false;
  }
 ...
 */
bool iContext::isAContextCurrentForThisThread() { return wglGetCurrentContext() == NULL ? false : true; }
...
bool iContext::setDecoration(ACTION _action) {
  ...
  switch (_action) {
    ...
    default:
      eLOG(
          "This message is theoretically totally impossible! [bool iContext::setDecoration( "
          "ACTION _action )]");
      return false;
  }
