...
LRESULT CALLBACK eContext::initialWndProc( HWND _hwnd, UINT _uMsg, WPARAM _wParam, LPARAM _lParam ) {
   if ( _uMsg == WM_NCCREATE ) {
      ...
      eEventInfo _tempInfo( e_engine::e_engine_internal::__eInit_Pointer_OBJ.get() );
...
LRESULT CALLBACK eContext::actualWndProc( UINT _uMsg, WPARAM _wParam, LPARAM _lParam, eEventInfo _tempInfo ) {
   ...
   switch ( _uMsg ) {
      ...
      case WM_MOUSEMOVE: //Mouse moved, see http://msdn.microsoft.com/en-us/library/windows/desktop/ms645616%28v=vs.85%29.aspx
         _tempInfo.type          = E_EVENT_MOUSE; 
         _tempInfo.eMouse.posX   = GlobConf.win.mousePosX = _lParam & 0xFFFF; // Get the low order word as the x-Position
         _tempInfo.eMouse.posY   = GlobConf.win.mousePosY = _lParam >> 16; // Get the high order word as the y-Position
      ...
      case WM_LBUTTONDOWN:
         ...
         _tempInfo.eMouse.posX   = GlobConf.win.mousePosX;
         _tempInfo.eMouse.posY   = GlobConf.win.mousePosY;
