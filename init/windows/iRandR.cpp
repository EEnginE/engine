...
void iRandR::printRandRStatus() {
   ...
   for( iDisplays const & d : vCurrentConfig_eD ) {
      ...
      iLOG(
            "Display: ", d.getName(),
            "\n  - Primary: ", d.getIsPrimary(),
            "\n  - Current: ", lWidth_uI, "x", lHeight_uI, ":", lRate_D
      );

      for( DEVMODEW const & D : d.vModes_V_win32 ) {
         iLOG( "Resolution: ", (int32_t)D.dmPelsWidth, "x", (int32_t)D.dmPelsHeight, ":", (double)D.dmDisplayFrequency );
         iLOG(
               "\n  - DM_POSITION           ", D.dmFields & DM_POSITION           ? 1 : 0, ": ", (int32_t)D.dmPosition.x, "x", (int32_t)D.dmPosition.y,
               "\n  - DM_BITSPERPEL         ", D.dmFields & DM_BITSPERPEL         ? 1 : 0, ": ", (int32_t)D.dmBitsPerPel,
               "\n  - DM_PELSWIDTH          ", D.dmFields & DM_PELSWIDTH          ? 1 : 0, ": ", (int32_t)D.dmPelsWidth,
               "\n  - DM_PELSHEIGHT         ", D.dmFields & DM_PELSHEIGHT         ? 1 : 0, ": ", (int32_t)D.dmPelsHeight,
               "\n  - DM_DISPLAYFREQUENCY   ", D.dmFields & DM_DISPLAYFREQUENCY   ? 1 : 0, ": ", (double)D.dmDisplayFrequency, "\n"
         );
      }
   }
}
...
bool iRandR::setPrimary( const iDisplays &_disp ) {
   ...
   switch( ChangeDisplaySettingsExW( _disp.getDisplayDevice().DeviceName, &lNotReallyNeeded, NULL, CDS_SET_PRIMARY, NULL ) ) {
      ...
      case DISP_CHANGE_RESTART :
         wLOG( "ChangeDisplaySettingsEx returned DISP_CHANGE_FAILED [setPrimary] (You need to restart yout PC because you have Windows)" );
         return false;
...
bool iRandR::applyNewRandRSettings() {
   ...
   for( iDisplays const & disp : vDisplaysToChange_eD ) {
      ...
      iLOG( "Change ", disp.getDisplayDevice().DeviceName, " to: ", (int32_t)lDevmodeToChange_win32.dmPelsWidth, "x", (int32_t)lDevmodeToChange_win32.dmPelsHeight );
      switch(
         ...
         case DISP_CHANGE_RESTART :
            wLOG( "ChangeDisplaySettingsEx returned DISP_CHANGE_FAILED [set] (You need to restart yout PC because you have Windows)" );
            break;
   ...
   switch( ChangeDisplaySettingsExW( NULL, NULL, NULL, 0, NULL ) ) {
      ...
      case DISP_CHANGE_RESTART :
         wLOG( "ChangeDisplaySettingsEx returned DISP_CHANGE_FAILED [apply] (You need to restart yout PC because you have Windows)" );
         break;
