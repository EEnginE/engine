 ...
 * \brief \b Structs: \a internal::_crtc, \a internal::_outpu , \a internal::_mode , \a
 * internal::_config
 */
...
struct _crtc {
   ...
   int posX; //!< 'posX' and 'posY' indicate the position of this CRTC within the screen region.
             //They will be set to 0 when the CRTC is disabled.
   int posY; //!< 'posX' and 'posY' indicate the position of this CRTC within the screen region.
             //They will be set to 0 when the CRTC is disabled.
   unsigned int width; //!< 'width' and 'height' indicate the size of the area within the screen
   ...
   unsigned int height; //!< 'width' and 'height' indicate the size of the area within the screen
                        //presented by this CRTC. This may be different than the size of the mode
                        //due to rotation, the projective transform, and the Border property
                        //described below.  They will be set to 0 when the CRTC is disabled.
   RRMode mode; //!< 'mode' indicates which mode is active, or None indicating that the CRTC has
                //been disabled and is not displaying the screen contents.
   Rotation rotation;  //!< 'rotation' indicates the active rotation. It is set to Rotate_0 when the
                       //CRTC is disabled.
   Rotation rotations; //!< 'rotations' contains the set of rotations and reflections supported by
                       //the CRTC.
   std::vector<RROutput> outputs; //!< 'outputs' is the list of outputs currently connected to this
                                  //CRTC and is empty when the CRTC is disabled.
   std::vector<RROutput> possibleOutputs; //!< 'possibleOutputs' lists all of the outputs which may
                                          //be connected to this CRTC.
};
...
struct _output {
   ...
   Time timestamp; //!< [not used]; 'timestamp' indicates when the configuration was last set.
   RRCrtc crtc; //!< 'crtc' is the current source CRTC for video data, or Disabled ( = None ) if the
                //output is not connected to any CRTC.
   std::string name; //!< 'name' is a UTF-8 encoded string designed to be presented to the user to
                     //indicate which output this is. E.g. "S-Video" or "DVI".
   unsigned long mm_width;  //!< 'widthInMillimeters' and 'heightInMillimeters' report the physical
                            //size of the displayed area. If unknown, or not really fixed (e.g., for
                            //a projector), these values are both zero.
   unsigned long mm_height; //!< 'widthInMillimeters' and 'heightInMillimeters' report the physical
                            //size of the displayed area. If unknown, or not really fixed (e.g., for
                            //a projector), these values are both zero.
   Connection connection;   //!< 'connection' indicates whether the hardware was able to detect a
   ...
   SubpixelOrder subpixel_order; //!< 'subpixel_order' contains the resulting subpixel order of the
                                 //connected device to allow correct subpixel rendering.
   std::vector<RRCrtc> crtcs;    //!< 'crtcs' is the list of CRTCs that this output may be connected
                              //to. Attempting to connect this output to a different CRTC results in
                              //a Match error.
   std::vector<RROutput> clones; //!< 'clones' is the list of outputs which may be simultaneously
   ...
   std::vector<RRMode> modes; //!< 'modes' is the list of modes supported by this output. Attempting
                              //to connect this output to a CRTC not using one of these modes
                              //results in a Match error.
   int npreferred; //!< The first 'npreferred' modes in 'modes' are preferred by the monitor in some
                   //way; for fixed-pixel devices, this would generally indicate which modes match
...
struct _mode {
   ...
   unsigned long
         dotClock; //!< The dotClock (more information: https://de.wikipedia.org/wiki/Pixeltakt)
