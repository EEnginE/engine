/*!
 * \file x11/RandR/eRandR_reload.cpp
 ...
 *
 * \returns \a true when everything went fine and \a false when it is impossible to set the display like this.
 *
...
bool eRandR::setDisplaySizes( eDisplays const &_disp ) {
   ...
   if ( ! lModeSupported_B && _disp.getMode() != None ) {
      // This should generally never happen because of eRandRDisplay! Something went wrong!!!
   ...
   if ( _disp.getIsEnabled() && lSingleMatch_XRR == None && lClonesMatch_XRR == None ) {
      ...
      if ( _disp.getClones().empty() ) {
         wLOG "RandR: Impossible to enable the Output " ADD _disp.getOutput() ADD " without cloning --> do nothing ( return false (eRandR::setDisplaySizes))" END
         return false;
      }
