   ...
   try {
      if ( boost::filesystem::exists( vert ) ) {    // Look for the vertex-shader
         if ( boost::filesystem::is_regular_file( vert ) ) {
            rShader s( temp[VERT], GL_VERTEX_SHADER );
            shaders.push_back( s );
            found_shaders++;
         }
      }
      if ( boost::filesystem::exists( frag ) ) {    // Look for the fragment-shader
         if ( boost::filesystem::is_regular_file( vert ) ) {
            rShader s( temp[FRAG], GL_FRAGMENT_SHADER );
            shaders.push_back( s );
            found_shaders++;
         }
      }
      if ( boost::filesystem::exists( geom ) ) {    // Look for the geometry-shader
         if ( boost::filesystem::is_regular_file( geom ) ) {
            rShader s( temp[GEOM], GL_GEOMETRY_SHADER );
            shaders.push_back( s );
            found_shaders++;
         }
      }
...
/*!
       ...
       */
unsigned int rLinker::test_program() {
   int status;
 ...
 */
void rLinker::setAttributes( unsigned int n, ... ) {
   attributes.clear();
   va_list list;
   va_start( list, n );
   for ( unsigned int i = 0; i < n; i++ ) {
