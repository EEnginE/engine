 ...
 * \vkIntern
 */
bool rPipeline::create(VkDevice _device, VkRenderPass _renderPass, uint32_t _subPass, VkPipelineCache _cache) {
  if (!isReadyToCreate()) {
 ...
 * \vkIntern
 */
uint32_t rPipeline::getVertexBindPoint() { return vShader->getVertexInputBindingDescription().binding; }
