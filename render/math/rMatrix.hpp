...
struct rMatrixData<T, 2, 1> {
  ...
  void normalize() {
    ...
    if (lLength2 > static_cast<T>(1.0 + TOLERANCE) && lLength2 < static_cast<T>(1.0 - TOLERANCE))
...
rMatrix<TYPE, ROWS, COLLUMNS>::rMatrix(TYPE *_f) {
  ...
  for (uint32_t i = 0; i < (ROWS * COLLUMNS); ++i) vDataMat[i] = _f[i];
}

template <class TYPE, uint32_t ROWS, uint32_t COLLUMNS>
rMatrix<TYPE, ROWS, COLLUMNS>::rMatrix(const rMatrix<TYPE, ROWS, COLLUMNS> &_newMatrix) {
  for (uint32_t i = 0; i < (ROWS * COLLUMNS); ++i) vDataMat[i] = _newMatrix.vDataMat[i];
}
...
rMatrix<TYPE, ROWS, COLLUMNS> &rMatrix<TYPE, ROWS, COLLUMNS>::operator=(rMatrix<TYPE, ROWS, COLLUMNS> _newMatrix) {
  for (uint32_t i = 0; i < (ROWS * COLLUMNS); ++i) vDataMat[i] = _newMatrix.get(i);
  return *this;
}
...
rMatrix<TYPE, ROWS, COLLUMNS> &rMatrix<TYPE, ROWS, COLLUMNS>::operator*=(const TYPE &_rScalar) {
  for (uint32_t i = 0; i < (ROWS * COLLUMNS); ++i) vDataMat[i] *= _rScalar;
...
typename std::enable_if<ROWS == COLLUMNS, DTTSEIW>::type rMatrix<TYPE, ROWS, COLLUMNS>::toIdentityMatrix() {
  vDataMat[0] = 1;
...
void rMatrix<TYPE, ROWS, COLLUMNS>::fill(TYPE &&_f) {
  for (uint32_t i = 1; i < ROWS * COLLUMNS; ++i) vDataMat[i] = _f;
}
...
void rMatrix<TYPE, ROWS, COLLUMNS>::downscale(rMatrix<TYPE, R, C> *_new) const {
  ...
  for (uint32_t X = 0; X < C; ++X)
    for (uint32_t Y = 0; Y < R; ++Y) _new->set(X, Y, get(X, Y));
}
...
void rMatrix<TYPE, ROWS, COLLUMNS>::upscale(rMatrix<TYPE, R, C> *_new) const {
  ...
  for (uint32_t x = 0; x < COLLUMNS; ++x)
    for (uint32_t y = 0; y < ROWS; ++y) _new->set(x, y, get(x, y));
}
...
void rMatrix<TYPE, ROWS, COLLUMNS>::multiply(const rMatrix<TYPE, COLLUMNS, COLLUMNS_NEW> &_matrix,
  ...
  for (uint32_t i = 0; i < COLLUMNS_NEW; ++i) { // Second Matrix
    for (uint32_t j = 0; j < ROWS; ++j) {       // First Matrix
      for (uint32_t k = 0; k < COLLUMNS; ++k)   // Both Matrices, this calculates the sum given to the target Matrix
...
void rMatrix<TYPE, ROWS, COLLUMNS>::add(const rMatrix<TYPE, ROWS, COLLUMNS> &_matrix,
  ...
  for (uint32_t i = 0; i < (ROWS * COLLUMNS); ++i) _targetMatrix->set(i, (vDataMat[i] + _matrix.get(i)));
}
...
void rMatrix<TYPE, ROWS, COLLUMNS>::setMat(ARGS &&... _args) {
  static_assert(sizeof...(_args) == (ROWS * COLLUMNS),
...
void rMatrix<TYPE, ROWS, COLLUMNS>::print(std::string _name, char _type) {
  ...
  for (size_t row = 0; row < ROWS; ++row) {
    lRowStr.clear();
    for (size_t collumn = 0; collumn < COLLUMNS; ++collumn) { TYPE2String((collumn * ROWS) + row, lRowStr); }
